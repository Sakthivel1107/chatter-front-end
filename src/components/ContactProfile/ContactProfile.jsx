...
const ContactProfile = () => {
  ...
  const handleDeleteContact = async () => {
          try{
              const response = await removeContact(contactProfile.id);
      ...
      const handleDeleteAllMessages = async () => {
          try{
              const response = await deleteAllMessages(contactProfile.id);
              if(response.status === 200){
                  setContacts(prev => prev.map(contact => contact.id === contactProfile.id ? {...contact,messages:[]}:contact));
      ...
      const handleBlockContact = async () => {
          const response = await blockContact(contactProfile.id);
          try {
              if(contactProfile.id === chatPageActiveUser.id){
                setChatPageActive(false);
              }
  ...
  return (
    <div className={viewContactProfile?"cp-overlay":"d-none"}>
      <div className='d-flex flex-column gap-2 align-items-center'>
        <i className='bi bi-arrow-left ms-auto me-2 mt-2 fs-3 rc' style={{color:"rgb(8, 240, 163)",WebkitTextStroke:"0.3px currentColor"}} onClick={() => setViewContactProfile(false)}></i>
        <div className='d-flex flex-column gap-1 align-items-center' style={{width:'100%'}}>
                <img src={contactProfile.url} alt={contactProfile.name} className='contact-profile-img rounded-circle'/>
                <h5 className='fw-bold'>{contactProfile.name}</h5>
                <small className='online-status' style={{color:"rgb(8, 240, 163)"}}>{contactProfile.online?"Online":(<small className='text-dark'>last seen: {formatTimestamp(contactProfile.lastSeen)}</small>)}</small>
        </div>
        <div className='d-flex flex-column gap-1 align-items-center pt-1' style={{width:'100%'}}>
            <div className='d-flex gap-3 cp-box' style={{width:'90%'}}>
                <i className='bi bi-person fs-4 cp-icon rounded-circle px-2 pt-1 mt-1'></i>
                <div>
                    <small className='text-secondary'>Name</small>
                    <h6>{contactProfile.name}</h6>
                </div>
            </div>
        </div>
        <div className='d-flex flex-column gap-1 align-items-center pt-1' style={{width:'100%'}}>
            <div className='d-flex gap-3 cp-box' style={{width:'90%'}}>
                <i className='bi bi-at fs-4 cp-icon rounded-circle px-2 pt-1 mt-1'></i>
                <div>
                    <small className='text-secondary'>Uid</small>
                    <h6>{contactProfile.uid}</h6>
                </div>
            </div>
        </div>
        <div className='d-flex flex-column gap-1 align-items-center pt-1' style={{width:'100%'}}>
            <div className='d-flex gap-3 cp-box' style={{width:'90%'}}>
                <i className='bi bi-globe fs-4 cp-icon rounded-circle px-2 pt-1 mt-1'></i>
                <div>
                    <small className='text-secondary'>Language</small>
                    <h6>{contactProfile.language}</h6>
                </div>
            </div>
        </div>
        <div className='d-flex flex-column gap-1 align-items-center pt-1' style={{width:'100%'}}>
